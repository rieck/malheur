...
B<malheur> [B<-hrvV>] [B<-m> F<maldir>] [B<-o> F<outfile>] I<action> F<dataset>
...
This option specifies the malheur directory F<maldir> which holds the
...
This option specifies the output file F<outfile> for analysis. The
...
reports are not sequential, this parameter need to be set to 1. In all
...
determines clusters. The clustering results are written to the file
...
incremental analysis. First, the archive "dataset1.zip" is analysed
where the internal state is reset using the option "-r". The the files
"dataset2.zip" and "dataset3.zip" are analysed and the incremental
